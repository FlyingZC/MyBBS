(function($) {
	$.fn.pager = function(options) {
		...
		return this.each(function() { // empty out the destination element and
										// then render out the pager with the
										// supplied options
	...
	function renderpager(pagenumber, pagecount,totalcount,buttonClickCallback) { 
		...
		$pager.append(renderButton('首页', pagenumber, pagecount,
				buttonClickCallback)).append(renderButton('上一页', pagenumber,
		...
		$pager.append(thpoint).append(renderButton('下一页', pagenumber,
				pagecount, buttonClickCallback)).append(renderButton('末页',
				pagenumber, pagecount, buttonClickCallback));
		$pager.append("<li class='thpoint' style='margin-top:0px;font-size:13px;'>共有"+totalcount+"条记录/每页显示15条记录/共"
				+ pagecount + " 页</li>");
		var strgoto = $("<li class='thpoint'>当前<input type='text' value='"
				+ pagenumber
				+ "'maxlength='6' id='gotoval' style='width:20px; height:16px;margin-top:-3px;padding-top:2px;padding-left:3px;'/>页</li>");
		$pager.append(strgoto);
		$pager.append(changepage('go', pagecount, buttonClickCallback));
		return $pager;
	} // renders and returns a 'specialized' button, ie 'next', 'previous'
		// etc. rather than a page number button
